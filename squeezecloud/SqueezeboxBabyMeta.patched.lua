-- SqueezeCloud patch: redirect SN hostname to custom server
-- Replace SQUEEZECLOUD_HOSTNAME below with your server address.
-- Examples:
--   Local Python server:           192.168.1.43
--   Cloudflare Worker (hostname):  squeezecloud.YOUR_SUBDOMAIN.workers.dev
--
-- After editing, deploy to the device with deploy_lua_patch.sh:
--   bash deploy_lua_patch.sh 192.168.1.72 [SERVER_IP] [CUSTOM_HOSTNAME]
...
--        jive.perfhook(1000, 10000)
	...
	-- SqueezeCloud patch: point SN hostname to our custom server.
	-- The deploy_lua_patch.sh script replaces SQUEEZECLOUD_HOSTNAME
	-- automatically when deploying to the device.
	-- If you see "CONFIGURE_ME" in the logs, the patch was deployed
	-- without running through the deploy script.
	jnt:setSNHostname("CONFIGURE_ME.squeezecloud.invalid")
