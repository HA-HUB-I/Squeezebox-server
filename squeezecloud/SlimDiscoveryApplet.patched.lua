...
function _discover(self)
	...
	if System:getUUID() then
		...
		-- SqueezeCloud patch: port replaced by deploy_lua_patch.sh.
		-- Default 9000 keeps the file valid Lua without patching;
		-- deploy script substitutes SQUEEZECLOUD_SN_PORT → 80 for
		-- Cloudflare Worker, or leaves 9000 for a local Python server.
		self:_serverUpdateAddress(squeezenetwork, jnt:getSNHostname(), 9000 --[[SQUEEZECLOUD_SN_PORT]], "mysqueezebox.com")
	end
